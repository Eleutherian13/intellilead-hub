...
export function LeadCard({ lead, onClick }) {
  ...
  // Support both API format (score as number) and mock format (score.total)
  const scoreValue =
    typeof lead.score === "number" ? lead.score : lead.score?.total || 0;

  // Support both API format (company as populated obj or just companyName) and mock format
  const companyName =
    lead.companyName ||
    lead.company?.normalizedName ||
    lead.company?.name ||
    "Unknown";
  const companyIndustry = lead.company?.industry || "";
  const companyState =
    lead.territory || lead.company?.locations?.[0]?.state || "Unknown";

  // Support both API product format and mock format
  const products = (lead.inferredProducts || []).map((p) => ({
    name: p.productName || p.product || p.productCode || "",
    ...
    >
      ...
      >
        ...
        <div
          className={cn(
            "priority-indicator",
            (priorityStyles[lead.priority] || priorityStyles.medium).indicator,
          )}
        />
        ...
        <div className="p-5 pl-6">
          ...
          <div className="flex items-start justify-between mb-4">
            ...
            >
              <span
                className={cn("text-xl font-bold", getScoreColor(scoreValue))}
              >
                {scoreValue}
              </span>
