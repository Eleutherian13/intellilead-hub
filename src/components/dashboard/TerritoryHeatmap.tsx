...
export function TerritoryHeatmap({ data }: TerritoryHeatmapProps) {
  const maxLeads = Math.max(...data.map((d) => d.leads));
  
  const getHeatColor = (leads: number) => {
  ...
  const getTextColor = (leads: number) => {
    const intensity = leads / maxLeads;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        {data.map((territory, idx) => (
          ...
          >
            <p className={cn("text-sm font-medium truncate", getTextColor(territory.leads))}>
              {territory.state}
            </p>
            <p className={cn("text-2xl font-bold", getTextColor(territory.leads))}>
              {territory.leads}
            </p>
            <p className={cn("text-xs opacity-80", getTextColor(territory.leads))}>
              leads
            </p>
          </motion.div>
        ))}
